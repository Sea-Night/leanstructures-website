...
- **Crediting an architect**: add `architect: { name, url }` to the project in `lib/projects.ts`, and put the literal token `{architect}` in that project's `meta` string wherever the name should sit (e.g. `'Structural design for {architect} & Forestry England'`). `ExpandedProjectView.tsx` splices in `components/ArchitectLink.tsx` there automatically — bold, ~8% larger, a slow infinite rose↔teal color fade, linked out to their site. This is the house style for architect credits; don't hand-roll a different treatment elsewhere.
...
- `components/bento/use-grid-dimensions.ts` — breakpoint tiers 7×5 down to a 2×6 floor (below 400px, i.e. most phones in portrait). The floor tier renders at a fixed comfortable row height and grows taller rather than shrinking to fit one screen — `MosaicGrid.tsx`'s floor branch is a natural-height block, not `flex-1`, and that "let mobile grow taller" override cascades up through `PageSwipeNav`/`PageTransition`/`app/projects/page.module.css`'s `.stage` (all `flex-1 min-h-0` only from `md:` up). There used to be a `use-floor-scale.ts` shrink-to-fit hook here; it was retired when the floor stopped shrinking tiles.
